...
const all_child_tabs = document.querySelectorAll('.customizer-list.childs-list');
...
function updatePrice() {
  ...
  priceContainers.forEach(container => {
    container.innerHTML = `<span>Total: Rs ${totalCharges.toLocaleString()}</span>`;
    container.setAttribute("customizer-mono-price", monoCharges.toFixed());
    container.setAttribute("total-price", totalCharges.toLocaleString());
  });
...
function hasOptions(restrictedOptionIds = [], restricted_option_Pid, selectedParentId, why_not_name) {
  ...
  if (li.getAttribute('hasdisabled-to') === selectedParentId) {
  ...
  function goToPrev() {
    if (currentIndex > 0) {
      ...
      if (mono_prev_tab && mono_prev_tab.classList.contains('mono-change')) {
        ...
        //   Add charges from selected radios
        document.querySelectorAll('input[type="radio"]:checked').forEach(checkedInput => {
          const charge = parseFloat(checkedInput.getAttribute('more-charges'));
          if (!isNaN(charge)) {
            totalCharges += charge;
...
//   // Price Update Logic
  ...
  priceContainers.forEach(container => {
    container.innerHTML = `<span>Total: Rs ${totalCharges.toFixed(0)}</span>`;
  });
